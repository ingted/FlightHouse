...
namespace P2PNET.TransportLayer
{
    public class BaseStation
    {
        ...
        public async void IncomingMsgAsync2(object sender, MsgReceivedEventArgs e)
        {
            //check if message is from this peer
            if (e.RemoteIp == this.LocalIpAddress)
            {
            ...
            if (e.BindingType == TransportType.UDP)
            {
                ...
                {
                    //not a known peer
                    await DirectConnectTCPAsync2(remotePeeripAddress);
                }
            }
        ...
        public async Task DirectConnectTCPAsync2(string ipAddress)
        {
            //send connection request
            TcpSocketClient senderTCP = new TcpSocketClient();
            ...
            try
            {
                await senderTCP.Socket.ConnectAsync(ipAddress, this.portNum);
            }
            catch (Exception e1)
            {
                throw e1;
